...
//Data from the JSON file is stored here
  const [Jupiter] = useState<any>(Data.Jupiter);
  const [value, setValue] = useState<number>(0);

  const { name, overview, content, source, structure, geology, rotation, revolution, radius, temperature, images, planet, internal } = Jupiter[value];
        ...
        <article>
          ...
          <div className='box-two'>
            <div className='wrap-buttons'>
              <button onClick={handleClick} data-value="overview" className={`overview-box overview ${index == overview ? "jupiter-active" : ""}`}><span className='overview-number'>01</span>Overview</button>
              <button onClick={handleClick} data-value="structure" className={`overview-box structure ${index == structure ? "jupiter-active" : ""}`}><span className='overview-number'>02</span>Internal structure</button>
              <button onClick={handleClick} data-value="geology" className={`overview-box geology ${index == geology ? "jupiter-active" : ""}`}><span className='overview-number'>03</span>Surface geology</button>
            </div>
          </div>
        </article>
      </section>
