...
const Uranus: React.FC = () => {
  ...
  const [Uranus] = useState<any>(Data.Uranus);
  const [value, setValue] = useState<number>(0);

  const { name, overview, content, source, structure, geology, rotation, revolution, radius, temperature, images, planet, internal } = Uranus[value];
        ...
        <article>
          ...
          <div className='box-two'>
            <div className='wrap-buttons'>
              <button onClick={handleClick} data-value="overview" className={`overview-box overview ${index == overview ? "uranus-active" : ""}`}><span className='overview-number'>01</span>Overview</button>
              <button onClick={handleClick} data-value="structure" className={`overview-box structure ${index == structure ? "uranus-active" : ""}`}><span className='overview-number'>02</span>Internal structure</button>
              <button onClick={handleClick} data-value="geology" className={`overview-box geology ${index == geology ? "uranus-active" : ""}`}><span className='overview-number'>03</span>Surface geology</button>
            </div>
          </div>
        </article>
      </section>
